...
> **Single Source of Truth**: 本文档以 `src/lib/types.ts` 与 `src/server/feishu/*` 的实现为准，如遇冲突，以代码为准。
...
| `locationName` | string? | 关联地点名（前端/编辑器派生展示） | `地点名称` |
...
2. **Sync Script (`npm run sync`)**:
   ...
   - 生成 `src/config/locations.json` 与 `src/data/content.json`。
3. **Frontend**: 组件直接读取 `content.json` 渲染。

## 状态管理
全局状态由 Zustand (`src/lib/store.ts`) 管理：
